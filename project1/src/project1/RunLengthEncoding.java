...
public class RunLengthEncoding implements Iterable {
  ...
  public RunLengthEncoding(int width, int height) {
    // Your solution here.
	  ...
	  this.height=height;
	  runList=new List();
	  runList.insertTail(0,0,0,width*height);
  ...
  public RunLengthEncoding(int width, int height, int[] red, int[] green,
    ...
    // Your solution here.
	  ...
	  for(int i=0;i<red.length;i++){
		  runList.insertTail(red[i],green[i],blue[i],runLengths[i]);
	  }
  ...
  public PixImage toPixImage() {
    ...
    ListNode curr=runList.head;
    int count=0;
    
    for(int j=0;j<height;j++){
    	for(int i=0;i<width;i++){
    		res.setPixel(i,j,(short)curr.r, (short)curr.g, (short)curr.b);
    		count++;
    		if(count==curr.len){
    			count=0;
    			curr=curr.next;
  ...
  public RunLengthEncoding(PixImage image) {
    ...
    // at the end.
	  ...
	  for(int j=0;j<height;j++){
		  for(int i=0;i<width;i++){
			  if(i==0 && j==0){
				  curr=new ListNode(image.getRed(i, j), image.getGreen(i, j),image.getBlue(i, j), 1);
			  }
			  else if(image.getRed(i,j)==curr.r && image.getGreen(i,j)==curr.g && image.getBlue(i,j)==curr.b){
				  curr.len++;
			  }
			  else{
				  runList.insertTail(curr.r,curr.g,curr.b,curr.len);
				  curr=new ListNode(image.getRed(i, j), image.getGreen(i, j),image.getBlue(i, j), 1);
			  }
		  }
	  }
  ...
  public void check() {
    // Your solution here.
	  if(runList.size<1){
		  System.out.println("Error:run list size less than one");
	  }
	  else if(runList.size==1){
		  if(runList.head.len!=width*height){
			  System.out.println("Error:the size is not equal");
		  }
	  }
	  else{
		  ListNode curr=runList.head;
		  ...
		  for(int i=1;i<runList.size;i++){
			  if(curr_next.r==curr.r && curr_next.g==curr.g && curr_next.b==curr.b){
				  System.out.println("Error:Consecutive runs have the same BGBs");
			  }
			  curr=curr_next;
			  curr_next=curr.next;
		  }
